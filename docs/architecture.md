...
The `scripts/sync.sh` script copies memory files to a sync folder. Changes propagate automatically through the cloud service. The script uses rsync for efficient incremental sync.
